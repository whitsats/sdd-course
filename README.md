...
> 一条主线：**GitHub Spec Kit**。三个案例：从小到大、从「规格优先」走到「规格即源」。
...
四个阶段、13 节课，每节 **1–2 小时**，每节都有可提交的产出物。

> ⏱️ **时间预算分两层，别用错**：跟着教程走一遍约 **24 小时**；
> 三个项目都能端到端演示约 **50 小时**——差出来的时间全在写代码、跑测试、跟 CI 打架上，**而那才是真正的价值所在**。
> 时间不够时的减法优先级见 [PLAN.md 附录](./PLAN.md)。

> 🔍 **计划经过一次审阅修订（v2）**，修掉了四个结构性缺陷：时间预算低估、L11 内容超载、项目 B 范围过大、以及两个真实缺口（规格变更管理、converge 不收敛）。修订记录见 [PLAN.md 末尾](./PLAN.md)。
...
阶段三 · 生产级规格锚定（4 节）                │  项目 B：taskflow（TS API）
  L06 Clarify & Checklist ──► L07 架构与契约规格
        ...
        └──► L08 Analyze & 分批 Implement ──► L09 Converge & 漂移治理 ⭐**达标节点**
                                              │
阶段四 · 进阶与自主化（3 节）                  │  项目 C：rulesmith（规格即源）
  L10 规格即源 ──► L11 生态对标 & 存量改造 ──► L12 团队化落地
```
...
不搞玩具 demo。三个项目是**同一件事的三个严谨度等级**，你会亲手体验「多花的那点规格成本」换来了什么。
...
.
├── README.md              ← 你在这里（课程主页）
├── PLAN.md                ← 详细学习计划：13 节全展开
├── lessons/               ← 教程正文，13 节全部就绪（19 个文件 / 5700+ 行）
...
│   ├── glossary.md        中英术语对照
│   ├── anti-patterns.md   SDD 反模式清单（28 条）
